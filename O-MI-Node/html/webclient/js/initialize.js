...
(function() {
  ...
  constsExt = function($, parent, util) {
    ...
    openOdfContextmenu = function(target) {
      ...
      createNode = function(particle, odfName, treeTypeName, defaultId) {
        return {
          ...
          _disabled: my.odfTree.settings.types[target.type].valid_children.indexOf(treeTypeName) === -1, // if not exists in valid_children 
    ...
    // $ ->
    fallback.ready(['jQuery', 'CodeMirror', 'codeMirrorExtension'], function() {
      ...
      my.ui = {
        ...
        ttl: basicInput('#ttl', function(a) { // double
          return (v.or(v.greaterThanEq(0), v.equals(-1)))(v.number(v.nonEmpty(a)));
        }),
        callback: basicInput('#callback', v.url), // Maybe string
        requestID: basicInput('#requestID', function(a) { // Maybe int
        ...
        interval: basicInput('#interval', function(a) { // Maybe number
          return (v.or(v.greaterThanEq(0), v.equals(-1), v.equals(-2)))(v.number(v.nonEmpty(a)));
        }),
        newest: basicInput('#newest', function(a) { // Maybe int
          return (v.greaterThan(0))(v.integer(v.number(v.nonEmpty(a))));
        }),
        oldest: basicInput('#oldest', function(a) { // Maybe int
          return (v.greaterThan(0))(v.integer(v.number(v.nonEmpty(a))));
        }),
        begin: $.extend(basicInput('#begin'), { // Maybe Date # TODO: merge duplicate datepicker code with the "end" below
          set: function(val) {
        ...
        end: $.extend(basicInput('#end'), { // Maybe Date
          set: function(val) {
            return this.ref.data("DateTimePicker").date(val);
          },
          get: function() {
  ...
  window.WebOmi.consts.afterJquery(function() {
    return window.Initialize = "ready";
  });

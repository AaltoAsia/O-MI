...
case class OdfObjects(
  ...
  def get(path: Path): Option[OdfNode] = {
    ...
    val grouped = objects.groupBy(_.path).mapValues {
      _.headOption.getOrElse(throw new Exception("Pathless Object was grouped."))
    }
...
case class OdfObject(
                      ...
                      attributes: Map[String, String] = HashMap.empty
                    ) extends OdfObjectImpl(id, path, infoItems, objects, description, typeValue, attributes) with
  OdfNode with
  Serializable {
  ...
  def get(path: Path): Option[OdfNode] = path match {
    ...
    case default: Path =>
      ...
      val grouped = haspaths.groupBy(_.path).mapValues {
        _.headOption.getOrElse(OdfObjects())
      }
      grouped.get(path) match {
        case None =>
  ...
  def withValues(pathValuesPairs: MutableMap[Path, OdfTreeCollection[OdfValue[Any]]]): OdfObject = this
    .withValues(pathValuesPairs.toMap)
  ...
  def update(
    ...
    this.copy(
      objects = objects map
        (o => if (o.path.isAncestorOf(spath)) o.update(spath, values, description, metaData) else o),
...
case class OdfInfoItem(
                        ...
                        typeValue: Option[String] = None,
                        attributes: Map[String, String] = HashMap.empty
                      ) extends OdfInfoItemImpl(path, values, description, metaData, typeValue, attributes) with
  OdfNode {

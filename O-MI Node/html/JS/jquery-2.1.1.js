		...
		(
				...
				function(window, noGlobal) {
					...
					jQuery
							.extend({
								...
								isPlainObject : function(obj) {
									// Not plain objects:
									// - Any object or value whose internal
									// [[Class]] property is not "[object
									// Object]"
									// - DOM nodes
									// - window
									...
									// |obj| is a plain object, created by {} or
								...
								globalEval : function(code) {
									...
									if (code) {
										// If the code includes a valid,
										// prologue position
										// strict mode pragma, execute code by
										// injecting a
										...
										if (code.indexOf("use strict") === 1) {
											...
											script.text = code;
											document.head.appendChild(script).parentNode
													.removeChild(script);
										} else {
											// Otherwise, avoid the DOM node
											// creation, insertion
											// and removal by using an indirect
											// global eval
											indirect(code);
										}
									}
					...
					var access = jQuery.access = function(elems, fn, key,
						...
						// Gets
						bulk ? fn.call(elems) : len ? fn(elems[0], key)
								: emptyGet;
					};
					...
					function Data() {
						// Support: Android < 4,
						// Old WebKit does not have
						// Object.preventExtensions/freeze method,
						// return new empty object instead with no [[set]]
						// accessor
					...
					Data.prototype = {
						key : function(owner) {
							// We can accept data for non-element nodes in
							// modern browsers,
						...
						set : function(owner, data, value) {
							...
							if (typeof data === "string") {
								...
								// Fresh assignments by object are shallow
								// copied
								if (jQuery.isEmptyObject(cache)) {
									jQuery.extend(this.cache[unlock], data);
									// Otherwise, copy the properties one-by-one
									// to the cache object
								} else {
									for (prop in data) {
						...
						get : function(owner, key) {
							// Either a valid cache is found, or will be
							// created.
							// New caches will be created and the unlock
							// returned,
							// allowing direct access to the newly created
							// empty data object. A valid owner object must be
							// provided.
						...
						access : function(owner, key, value) {
							...
							// 2. A string key was specified, but no value
							// provided
							//
							// Take the "read" path and allow the get method to
							// determine
							...
							if (key === undefined
								...
								return stored !== undefined ? stored : this
										.get(owner, jQuery.camelCase(key));
							}

							// [*]When the key is not a string, or both a key
							// and value
							// are specified, set or extend (existing objects)
							// with either:
							//
							...
							// Since the "set" path can have two possible entry
							// points
							// return the expected data based on which path was
							// taken[*]
						...
						remove : function(owner, key) {
							...
							if (key === undefined) {
								...
								// Support array or space separated string of
								// keys
								if (jQuery.isArray(key)) {
									...
									// argument path.
									name = key
											.concat(key.map(jQuery.camelCase));
								} else {
						...
						hasData : function(owner) {
							return !jQuery
									.isEmptyObject(this.cache[owner[this.expando]]
											|| {});
						},
					 ...
					 * 
					 * 1. Enforce API surface and semantic compatibility with
					 * 1.9.x branch 2. Improve the module's maintainability by
					 * reducing the storage paths to a single mechanism. 3. Use
					 * the same single mechanism to support "private" and "user"
					 * data. 4. _Never_ expose "private" data to user code
					 * (TODO: Drop _data, _removeData) 5. Avoid exposing
					 * implementation details on user objects (eg. expando
					 * properties) 6. Provide a clear path for implementation
					...
					jQuery.extend({
						...
						// TODO: Now that all calls to _data and _removeData
						// have been replaced
						// with direct calls to data_priv methods, these can be
						// deprecated.
							...
							.extend({
								data : function(key, value) {
									...
									return access(
											this,
											function(value) {
												var data, camelKey = jQuery
														.camelCase(key);

												// The calling jQuery object
												// (element matches) is not
												// empty
												// (and therefore has an element
												// appears at this[ 0 ]) and the
												// `value` parameter was not
												// undefined. An empty jQuery
												// object
												// will result in `undefined`
												// for elem = this[ 0 ] which
												// will
												// throw an exception if an
												// attempt to read a data cache
												// is made.
												if (elem && value === undefined) {
													// Attempt to get data from
													// the cache
													// with the key as-is
													data = data_user.get(elem,
															key);
					...
					jQuery.fn
							.extend({
								...
								promise : function(type, obj) {
									...
									while (i--) {
										tmp = data_priv.get(elements[i], type
												+ "queueHooks");
										if (tmp && tmp.empty) {
											count++;
					...
					(function() {
						var fragment = document.createDocumentFragment(), div = fragment
								.appendChild(document.createElement("div")), input = document
								.createElement("input");
					...
					jQuery.event = {
						...
						add : function(elem, types, handler, data, selector) {
							...
							var handleObjIn, eventHandle, tmp, events, t, handleObj, special, handlers, type, namespaces, origType, elemData = data_priv
									.get(elem);
							...
							if (!(eventHandle = elemData.handle)) {
								eventHandle = elemData.handle = function(e) {
									...
									return typeof jQuery !== strundefined
											&& jQuery.event.triggered !== e.type ? jQuery.event.dispatch
						...
						remove : function(elem, types, handler, selector,
							...
							var j, origCount, tmp, events, t, handleObj, special, handlers, type, namespaces, origType, elemData = data_priv
									.hasData(elem)
									&& data_priv.get(elem);
						...
						trigger : function(event, data, elem, onlyHandlers) {
							...
							var i, cur, tmp, bubbleType, ontype, handle, special, eventPath = [ elem
						...
						dispatch : function(event) {
							...
							var i, j, ret, matched, handleObj, handlerQueue = [], args = slice
									.call(arguments), handlers = (data_priv
									.get(this, "events") || {})[event.type]
						...
						fix : function(event) {
							...
							// All events should have a target; Cordova
							// deviceready doesn't
							if (!event.target) {
						...
						jQuery
								.each(
										...
										function(orig, fix) {
											...
											// Attach a single capturing handler
											// on the document while someone
					...
					function disableScript(elem) {
						elem.type = (elem.getAttribute("type") !== null) + "/"
								+ elem.type;
						return elem;
					}
					...
					function setGlobalEval(elems, refElements) {
						...
						for (; i < l; i++) {
							data_priv.set(elems[i], "globalEval", !refElements
									|| data_priv.get(refElements[i],
											"globalEval"));
						}
					}
					...
					function getAll(context, tag) {
						var ret = context.getElementsByTagName ? context
								.getElementsByTagName(tag || "*")
								: context.querySelectorAll ? context
										.querySelectorAll(tag || "*") : [];

						return tag === undefined || tag
								&& jQuery.nodeName(context, tag) ? jQuery
								.merge([ context ], ret) : ret;
					}
					...
					function fixInput(src, dest) {
						...
						// Fails to persist the checked state of a cloned
						// checkbox or radio button.
						if (nodeName === "input"
								&& rcheckableType.test(src.type)) {
							dest.checked = src.checked;

							// Fails to return the selected option to the
							// default selected state when cloning options
					...
					jQuery
							.extend({
								clone : function(elem, dataAndEvents,
										deepDataAndEvents) {
									var i, l, srcElements, destElements, clone = elem
											.cloneNode(true), inPage = jQuery
								...
								buildFragment : function(elems, context,
										scripts, selection) {
									var elem, tmp, tag, wrap, contains, j, fragment = context
											.createDocumentFragment(), nodes = [], i = 0, l = elems.length;
					...
					jQuery.fn
							.extend({
								...
								domManip : function(args, callback) {
									...
									var fragment, first, scripts, hasScripts, node, doc, i = 0, l = this.length, set = this, iNoClone = l - 1, value = args[0], isFunction = jQuery
											.isFunction(value);
					...
					var getStyles = function(elem) {
						return elem.ownerDocument.defaultView.getComputedStyle(
								elem, null);
					};
					...
					function curCSS(elem, name, computed) {
						...
						// Support: IE9
						// getPropertyValue is only needed for .css('filter') in
						// IE9, see #12537
						if (computed) {
						...
						if (computed) {
							...
							// iOS < 6 (at least) returns percentage for a
							// larger set of values, but width seems to be
							// reliably pixels
					...
					(function() {
						var pixelPositionVal, boxSizingReliableVal, docElem = document.documentElement, container = document
						...
						// Executing both pixelPosition & boxSizingReliable
						// tests require only one layout
						...
						// Don't assume that getComputedStyle is a property of
						// the global object
									...
									.extend(
											support,
											{
												...
												reliableMarginRight : function() {
													// Support: Android 2.3
													// Check if div with
													// explicit width and no
													// margin-right incorrectly
													// gets computed
													// margin-right based on
													// width of container.
													// (#3333)
													// WebKit Bug 13343 -
													...
													// only executed once so no
													// memoizing is needed.
													var ret, marginDiv = div
					...
					function getWidthOrHeight(elem, name, extra) {
						...
						var valueIsBorderBox = true, val = name === "width" ? elem.offsetWidth
								: elem.offsetHeight, styles = getStyles(elem), isBorderBox = jQuery
							...
							.extend({
								...
								style : function(elem, name, value, extra) {
									...
									if (value !== undefined) {
										...
										// correctly by specifying setters in
										// cssHooks,
								...
								css : function(elem, name, extra, styles) {
									var val, num, hooks, origName = jQuery
											.camelCase(name);
					...
					function defaultPrefilter(elem, props, opts) {
						...
						var prop, value, toggle, tween, hooks, oldfire, display, checkDisplay, anim = this, orig = {}, style = elem.style, hidden = elem.nodeType
								&& isHidden(elem), dataShow = data_priv.get(
								elem, "fxshow");
						...
						if (elem.nodeType === 1
							...
							// Record all 3 overflow attributes because IE9-10
							// do not
							...
							// "none"
							checkDisplay = display === "none" ? data_priv.get(
									elem, "olddisplay")
					...
					jQuery.fn
							.extend({
									...
									return this
											.each(function() {
												var dequeue = true, index = type != null
														&& type + "queueHooks", timers = jQuery.timers, data = data_priv
														.get(this);
								...
								finish : function(type) {
									...
									return this
											.each(function() {
												var index, data = data_priv
														.get(this), queue = data[type
					...
					(function() {
						var input = document.createElement("input"), select = document
						...
						// Check the default checkbox/radio value ("" on old
						// WebKit; "on" elsewhere)
						support.checkOn = input.value !== "";

						// Must access the parent to make an option select
						// properly
						// Support: IE9, IE10
					...
					// Support: IE9+
					// Selectedness for an option in an optgroup can be
					// inaccurate
					...
					jQuery.fn
							.extend({
								...
								removeClass : function(value) {
									var classes, elem, cur, clazz, j, finalValue, proceed = arguments.length === 0
											|| typeof value === "string"
											&& value, i = 0, len = this.length;
								...
								valHooks : {
									...
									select : {
										...
										set : function(elem, value) {
											...
											while (i--) {
												option = options[i];
												if ((option.selected = jQuery
														.inArray(option.value,
																values) >= 0)) {
													optionSet = true;
												}
											}
					...
					var xhrId = 0, xhrCallbacks = {}, xhrSuccessStatus = {
						...
						0 : 200,
						// Support: IE9
						// #1450: sometimes IE returns 1223 when it should be
						// 204
						1223 : 204
								...
								if (support.cors || xhrSupported
									...
									return {
										send : function(headers, complete) {
											...
											// as conditions for a preflight are
											// akin to a jigsaw puzzle, we
											// simply never set it to be sure.
											// (it can always be set on a
											// per-request basis or even using
											// ajaxSetup)
											// For same-domain requests, won't
											// change header if already
											// provided.
											...
											try {
												// Do send the request (this may
												// raise an exception)
												xhr
														.send(options.hasContent
																&& options.data
																|| null);
											} catch (e) {
												// #14683: Only rethrow if this
												// hasn't been notified as an
												// error yet
												if (callback) {
													throw e;
												}

...
CodeMirror.copyState(pers.mode, pers.state),
      ...
      innerMode: function(state) { return state.local && {mode: state.local.mode, state: 
state.localState}; },
  ...
  function enterLocalMode(config, state, spec, token) {
    ...
    if (spec.persistent && !pers)
      state.persistentStates = {mode: mode, spec: spec.spec, state: lState, next: 
state.persistentStates};
  ...
  function indentFunction(states, meta) {
    return function(state, textAfter, line) {
      ...
      if (state.indent == null || state.local || meta.dontIndentStates && indexOf(state.state, 
meta.dontIndentStates) > -1)

...
class TypesTest extends Specification {
  ...
  def e5 = new ResponseRequest(Seq(OmiResult("1","200"))).isInstanceOf[OmiRequest]

  def e6 = new CancelRequest(10.seconds, Seq()).isInstanceOf[OmiRequest]
  
  def e10 = !new OdfInfoItem(Seq(), Seq()).isInstanceOf[OmiRequest]
  
  def e11 = !new OdfObject(Seq(),Path("Objects/TypesTest"), Seq(), Seq()).isInstanceOf[OmiRequest]
  
  def e100 = new OdfInfoItem(Seq(), Seq()).isInstanceOf[OdfNode]

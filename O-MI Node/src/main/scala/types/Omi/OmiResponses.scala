...
object Responses{
  ...
  def Success(
    ...
    objects : Option[ImmutableODF] = None, 
  ...
  def NotFoundPaths( objects: ImmutableODF, ttl: Duration = 10.seconds ) : ResponseRequest =ResponseRequest(
  ...
  def Poll( requestID: RequestID, objects: ImmutableODF, ttl: Duration = 10.seconds) : ResponseRequest =ResponseRequest(
    OdfTreeCollection(
      Results.Poll(
        requestID,
        objects

...
import types.odf._
import types.omi._
import types.omi.Callback._ // implicit: String => Callback
...
object OMIParser extends parsing.Parser[OmiParseResult] {
  ...
  private[this] def parseRead(read: xmlTypes.ReadRequestType, ttl: Duration): OmiParseResult = {
    ...
    if(read.requestID.nonEmpty) {
      Right(Iterable(
        PollRequest(
          callback,
          Vector(read.requestID.map(parseRequestID):_*),
          ttl
        )))
    } else{
      read.msg match {
  ...
  private[this] def parseCancel(cancel: xmlTypes.CancelRequestType, ttl: Duration): OmiParseResult = {
    Right(Iterable(
      CancelRequest(
        Vector(cancel.requestID.map(parseRequestID):_*),
        ttl
      )
    ))
  }

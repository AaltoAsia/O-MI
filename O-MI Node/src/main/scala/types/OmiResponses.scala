...
object Responses{
  case class Success( requestID: Option[Long] = None, objects : Option[OdfObjects] = None, ttl: Duration = 0.seconds ) extends ResponseRequest{
    override val results: OdfTreeCollection[OmiResult] = OdfTreeCollection(Results.Success(requestID,objects))
  }
  case class NotImplemented( ttl: Duration = 0.seconds) extends ResponseRequest{
    override val results: OdfTreeCollection[OmiResult] = OdfTreeCollection(Results.NotImplemented())
  }
  case class Unauthorized( ttl: Duration = 0.seconds) extends ResponseRequest{
    override val results: OdfTreeCollection[OmiResult] = OdfTreeCollection(Results.Unauthorized())
  }
  case class InvalidRequest(msg: String = "", ttl: Duration = 0.seconds) extends ResponseRequest{
    override val results: OdfTreeCollection[OmiResult] = OdfTreeCollection(Results.InvalidRequest(msg))
  }
  case class InvalidCallback(callbackAddr: String, ttl: Duration = 0.seconds ) extends ResponseRequest{
    override val results: OdfTreeCollection[OmiResult] = OdfTreeCollection(Results.InvalidCallback(callbackAddr))
  }
  case class NotFoundPaths( paths: Vector[Path], ttl: Duration = 0.seconds ) extends ResponseRequest{
    override val results: OdfTreeCollection[OmiResult] = OdfTreeCollection(Results.NotFoundPaths(paths))
  }

  case class NotFoundRequestIDs( requestIDs: Vector[Long], ttl: Duration = 0.seconds ) extends ResponseRequest{
    override val results: OdfTreeCollection[OmiResult] = OdfTreeCollection(Results.NotFoundRequestIDs(requestIDs))
  }
  case class ParseErrors( errors: Vector[ParseError], ttl: Duration = 0.seconds ) extends ResponseRequest{
    override val results: OdfTreeCollection[OmiResult] = OdfTreeCollection(Results.ParseErrors(errors))
  }

  case class InternalError( message: String, ttl: Duration = 0.seconds ) extends ResponseRequest{
  ...
  object InternalError{
    ...
    def apply(e: Throwable ): InternalError = InternalError(e.getMessage(),0.seconds)
  }

  case class TimeOutError(message: String = "", ttl: Duration = 0.seconds) extends ResponseRequest{
    override val results: OdfTreeCollection[OmiResult] = OdfTreeCollection(Results.TimeOutError(message))
  } 
  case class Poll( requestID: Long, objects: OdfObjects, ttl: Duration = 0.seconds) extends ResponseRequest{
